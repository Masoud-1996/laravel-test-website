...
class LoginRegisterController extends Controller
{
    ...
    public function loginRegister(LoginRegisterRequest $request)
    {
        ...
        elseif (preg_match('/^(\+98|98|0)9\d{9}$/', $inputs['id'])) {
            ...
            $errorMessage = 'شناسه ورودی شما نه شماره موبایل است نه ایمیل';
            return redirect()->route('auth.customer.login-register-form')->withErrors(['id' => $errorMessage]);

        }

        if (empty($user)) {
            $newUser['password'] = '95457565';
    ...
    public function loginConfirm($token, LoginRegisterRequest $requset)
    {
        $inputs = $requset->all();
